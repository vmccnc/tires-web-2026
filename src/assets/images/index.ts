export { default as heroImage } from './hero.jpg';
export { default as fallBackProduct } from './fallbackProduct.jpg';
export { default as guaranteeBanner } from './guaranteeBanner.jpg';
export { default as supportBanner } from './supportBanner.png';
export { default as aboutUsBanner } from './aboutUsBanner.png';
export { default as helpBanner } from './helpBanner.png';
export { default as tireServiceBanner } from './tireServiceBanner.jpg';
export { default as team } from './team.jpg';
